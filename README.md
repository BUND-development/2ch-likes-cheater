...
2. так же `pip install requests pysocks urllib3 termcolor backoff` или скрипт попробует автоматически за вас это сделать  
...
`HOWMANYALLOW` - в каком диапазоне +- скрипт может пробовать фиксить потоки. Допилено.  
`MINTHREADSALLOW` - минимальное значение количества потоков, ниже которого скрипт не будет оптимизировать потоки  
В коде есть много комментариев, так что фанатские фиксы и допилы не составят труда.  
`MAXTRIES` - количество попыток отправки лайка  
